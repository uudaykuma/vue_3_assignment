<template>
  <div class="news-card" v-if="article.title !== '[Removed]'">
    <div class="profile-pic">
      <img :src="article.urlToImage || 'https://miro.medium.com/v2/resize:fit:1400/1*MXyMqcEJ6Se0SCWcYCKZTQ.jpeg'" alt="News Picture" />
    </div>
    <div class="news-info">
      <h2 class="title" @click="goToDetailPage">{{ article.title || "N/A" }}</h2>
    ...
    <div class="bookmark">
      <span :class="['bookmark-icon', { filled: isBookmarked }]" @click="handleBookmark">&#9733;</span>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.isBookmarked = bookmarks.some(item => item.title === this.article.title);
  },
  methods: {
    handleBookmark() {
      let bookmarks = JSON.parse(localStorage.getItem('myBookmark')) || [];
      const index = bookmarks.findIndex(item => item.title === this.article.title);
    ...
    goToDetailPage() {
      ...
      this.$router.push({ name: 'Detail', params: { title: this.article.title } });
    },
  },
};
</script>
